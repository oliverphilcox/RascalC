...
class Particle {
  public:
    ...
    uint reg; // The jackknife region of the particle
};
...
class PairsJack {
  ...
  public:
    ...
    PairsJack(uint _nbin, uint _njack) {
    	init(_nbin,_njack);
    }
  private:
    void init(uint _nbin, uint _njack) {
  	// Initialize the binning
    	nbin=_nbin;
    	njack=_njack;
    	len=nbin*njack*njack+1;

		posix_memalign((void **) &xi0, PAGE, sizeof(double)*len);
  ...
  public:
      ...
      void add(int b, Float dz,uint jack1,uint jack2, Float w) {
      	// Add up the weighted pair for the monopole and quadrupole correlation function
              xi0[jack1+jack2*njack+b*njack*njack] += w;
      }

      void sum_power(PairsJack *p) {
  	// Just add up all of the threaded pairs into the zeroth element
      ...
      void report_pairs() {
      	for (uint j1=0; j1<njack; j1++) {
          //	for (uint j2=0; j2<njack; j2++) {
              	for (uint b=0; b<nbin; b++) {
              		printf("Pairs %2d %2d %2d %9.0f\n", j1, j1, b, xi0[j1+j1*njack+b*njack*njack]);
              	}
          	//}
      	}
      }
...
void compute_jackpairs(Grid *grid, Float rmax) {
	...
	for (int j = primary.start; j<primary.start+primary.np; j++) {
		...
		for (int k = sec.start; k<sec.start+sec.np; k++) {
		    ...
		    */
		    ...
		    pairs[thread].add(bin, dx.z, primary_reg, grid->p[k].reg, grid->p[k].w*primary_w);
...
//	   fprintf(stderr,"%d>=%d %g %g %g %g\n",stat,rstart,tmp[0],tmp[1],tmp[2],tmp[3]);
	   ...
	   if(rstart>0&&j>=rstart)
		p[j].w = -tmp[stat-2];
	        ...
	        p[j].w = tmp[stat-2];
       if(stat==5){
    	   p[j].reg=(uint)tmp[stat-1];
       }
//	if(j%1000==0)
//	   fprintf(stderr,"%d %d %d %d %g %g\n",j,rstart,stat,p[j].reg,p[j].w,tmp[stat-1]);
