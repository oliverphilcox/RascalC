...
class Integrals{
public:
    CorrelationFunction *cf12, *cf13, *cf24, *cf23;
...
public:
    ...
    Integrals(Parameters *par, CorrelationFunction *_cf12, CorrelationFunction *_cf13, CorrelationFunction *_cf23, CorrelationFunction *_cf24, JK_weights *_JK12, JK_weights *_JK23, JK_weights *_JK34, int _I1, int _I2, int _I3, int _I4, Float* _product_weights12_12, Float* _product_weights12_23, Float* _product_weights12_34){
        cf12 = new CorrelationFunction(_cf12);
        cf13 = new CorrelationFunction(_cf13);
        cf23 = new CorrelationFunction(_cf23);
    ...
    inline void second(const Particle* pi_list, const int* prim_ids, int pln, const Particle pj, const int pj_id, int* &bin, Float* &xi_ij, Float* &wij, const double prob, const double prob1, const double prob2){
    ...
    inline void third(const Particle* pi_list, const int* prim_ids, const int pln, const Particle pj, const Particle pk, const int pj_id, const int pk_id, const int* bin_ij, const Float* wij, Float* &xi_ik, Float* &xi_ij, Float* wijk, const double prob){
        // Accumulates the three point integral C3. Also outputs an array of xi_ik and bin_ik values for later reuse.
        
        // First define variables:
        Particle pi;
        Float rik_mag, rik_mu, c3v, c3vj, rjk_mag, rjk_mu, tmp_weight, xi_ik_tmp, JK_weight,xi_jk_tmp;
        int tmp_bin, tmp_full_bin;
        
        cleanup_l(pj.pos,pk.pos,rjk_mag,rjk_mu); 
        tmp_bin = getbin(rjk_mag,rjk_mu); // bin for each particle
        
        xi_jk_tmp = cf23->xi(rjk_mag,rjk_mu);
        ...
        for(int i=0;i<pln;i++){ // Iterate ovr particle in pi_list
            ...
            // Now compute the integral;
            c3v = tmp_weight*pj.w/prob*4.*(xi_ik_tmp+1.-xi_ij[i]-xi_jk_tmp); // include symmetry factor
            c3vj = c3v*JK_weight;
    ...
    inline void fourth(const Particle* pi_list, const int* prim_ids, const int pln, const Particle pj, const Particle pk, const Particle pl, const int pj_id, const int pk_id, const int pl_id, const int* bin_ij, const Float* wijk, const Float* xi_ik, const double prob){
        ...
        for(int i=0;i<pln;i++){ // Iterate over particle in pi_list
            ...
            // Now compute the integral;
            c4v = tmp_weight/prob*(2.*xi_ik[i]*xi_jl+4.*xi_ik[i]); // with xi_ik*xi_jl = xi_il*xi_jk symmetry factor
            c4vj = c4v*JK_weight;
