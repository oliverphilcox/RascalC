...
// Define parameter structure for integrand (must be defined outside the class)
struct xi_integrand_params {Float R; Float n; CorrelationFunction *corr;};
        
double xi_integrand(double m, void * p){
    // Integrand for the 1d probability grid integral weighted by xi(r).
    // NB: This must be multiplied by a prefactor independent of the integration variable m.
        
    // Read in parameters
    struct xi_integrand_params * params = (struct xi_integrand_params *)p;
    double R = (params->R);
    double n = (params->n);
    CorrelationFunction corr = (params->corr);
        
    Float factor_1 = pow((m+n)/(2*R),2);
    Float factor_2 = pow((m-n)/(2*R),2);
        
    return m*corr.xi(m) / (R*n) * (exp(-factor_1)+exp(-factor_2));
    }
	...
	public:
		...
		if (par->loadname!=NULL&&(xin==NULL||np==0)){
			...
			if (xin==NULL||np==0){
                ...
                integData2(&x,&n,nside,boxside); // could change to 1/r^2 here by using f instead of f_corr and 1 instead of 0
                printf("# Probability grid computation complete\n");
                exit(1);
            }
			else
...
~RandomDraws2() {
	...
	private:
		...
		void integData2(double **x, long *np, int nside, double boxside){
...
#pragma omp parallel
        {            
            ...
            double n, result, error;
            
            // Set up integration workspace
            gsl_integration_workspace * w  = gsl_integration_workspace_alloc(100);
            
            // Use the gsl integration package to perform 1d integration over a semi-infinite range via the QAGI routine
            gsl_function FF;
            
            struct xi_integrand_params params;

            FF.function = &xi_integrand;
            ...
            for(int i = 0; i<=len;i++){
                for(int k = 0; k<=len;k++){
                    for(int l = 0;l<=len;l++){
                        ...
                        // Update integral passing parameters
                        params = {boxside/2, n, corr};
                        FF.params = &params;
                        
                        // Perform the integration
                        gsl_integration_qagiu(&FF, 0, 0, 1e-3, 100, w, &result, &error);
                        ...
                        for(int is=-1;is<=1;is+=2){
                            for(int ks=-1;ks<=1;ks+=2){
                                for(int ls=-1;ls<=1;ls+=2){
                                    ...
                                    int kc=ks*k+len;
                                    int lc=ls*l+len;
                                    (*x)[nside*nside*ic+nside*kc+lc]=result;
                                }
                            }
                        }
                    }
