...
class Grid {
  public:
    ...
    Float3 origin;  //Origin of the input from grid perspective
	...
	//Determine origin
	origin=(cellsize*((Float3)cell_id_from_1d(pos_to_cell(input[0].pos))+Float3(0.5,0.5,0.5)))
			+cell_centered_pos(input[0].pos)-input[0].pos;
...
#ifdef MBIN
    posix_memalign((void **) &xi0, PAGE, sizeof(double)*NBIN*MBIN);
...
#else
        xi0[MBIN*b+(int)floor(fabs(dz)*MBIN)] += w;
#endif
    }
...
inline Float get_mu(Float3 pos,Float3 los,Float norm){
	//Absolute value as mu bins defined in terms of |mu|
	//Assumes that pos is already normalized
	return fabs(pos.dot(los)/los.norm());
}
...
#ifdef OPENMP
    cpu_set_t mask[MAXTHREAD+1];
	int tnum=0;
	sched_getaffinity(0, sizeof(cpu_set_t),&mask[MAXTHREAD]);
	fprintf(stderr,"# CPUs used are: ");
	for(int ii=0;ii<64;ii++){
		if(CPU_ISSET(ii, &mask[MAXTHREAD])){
			fprintf(stderr,"%d ",ii);
			CPU_ZERO(&mask[tnum]);
...
#pragma omp parallel for schedule(dynamic,8) reduction(+:cnt)
	...
	Float3 primary_center_x2=2*(grid->cellsize*((Float3)prim_id+Float3(0.5,0.5,0.5))-grid->origin);
	...
	for (int j = primary.start; j<primary.start+primary.np; j++) {
		...
		// secondary cell.
#ifdef PERIODIC
		Float3 pplus=grid->p[j].pos + primary_center_x2 + grid->cell_sep(delta);
#endif
		...
		for (int k = sec.start; k<sec.start+sec.np; k++) {
		    ...
		    Float3 px = grid->p[k].pos + pplus;
#else
		    Float3 dx = grid->p[k].pos - grid->p[j].pos;
		    Float3 px = grid->p[k].pos + grid->p[j].pos;
#endif
...
#else
		    pairs[thread].add(bin, get_mu(dx,px,norm2), grid->p[k].w*primary_w);
#endif
