...
class Pairs {
  ...
  public:
...
#ifdef MBIN
    posix_memalign((void **) &xi0, PAGE, sizeof(double)*NBIN*NBIN*MBIN);
	...
	posix_memalign((void **) &xi4, PAGE, sizeof(double)*NBIN);
	posix_memalign((void **) &xi6, PAGE, sizeof(double)*NBIN);
    ...
    inline void add(int b1,int b2,int b3, Float w) {
	// Add up the weighted pair for the monopole and quadrupole correlation function
#ifndef MBIN
    	Float dzs=dz*dz;
    	xi0[b] += w;
    	xi2[b] += w*(3.0*dzs-1)*0.5;
        xi4[b] += w*(3. - 30.*dzs + 35.*dzs*dzs)/8.;
        xi6[b] += w*(-5 + 105*dzs - 315*dzs*dzs + 231*dzs*dzs*dzs)/16.;
#else
        xi0[NBIN*MBIN*b1+MBIN*b2+b3] += w;
#endif
    }
...
#ifndef MBIN
	for (int j=0; j<NBIN; j++) {
	    printf("Pairs %2d %9.0f %9.0f %9.5f %9.5f\n",
			j, xi0[j], xi2[j], xi4[j], xi6[j]);
	}
#else
	for (int i=0; i<NBIN; i++) {
		printf("Pairs %2d ",i);
		for (int j=0; j<NBIN; j++) {
			for (int k=0; k<MBIN; k++) {
				printf("%9.0f ",xi0[i*NBIN*MBIN+j*MBIN+k]);
			}
		}
		printf("\n");
	}
...
//		    dx = dx/norm2;
		    ...
		    // Then loop over tertiaries, cell-by-cell
		   			integer3 delta2;
		   			for (delta2.x = -maxsep; delta2.x <= maxsep; delta2.x++)
		   			for (delta2.y = -maxsep; delta2.y <= maxsep; delta2.y++)
		   			for (delta2.z = -maxsep; delta2.z <= maxsep; delta2.z++) {
		   			const int samecell2 = (delta2.x==0&&delta2.y==0&&delta2.z==0)?1:0;
		   			Cell thi = grid->c[grid->wrap_cell(prim_id+delta2)];
		   			Float3 ppos2 = grid->p[j].pos - grid->cell_sep(delta2);
		   			// This is the position of the primary particle as viewed from the
		   			// tertiary cell.
		   			// Now loop over the particles in this tertiary cell
		   				for (int l = thi.start; l<thi.start+thi.np; l++) {
		   					// Now we're considering these two particles!
		   					if (samecell2&&j==l) continue;   // Exclude self-count
		   					if (k==l) continue;   // Exclude self-count

		   					Float3 dy = grid->p[l].pos - ppos2;
		   					Float normy2 = dy.norm2();
		   					if (!(normy2<rmax2 && normy2>rmin2)) continue;

		   					Float3 dz = (grid->p[l].pos + grid->cell_sep(delta2)) - (grid->p[k].pos + grid->cell_sep(delta));
		   					Float normz2 = dz.norm2();
//		   					if (normz2<rmax2 && normz2>rmin2) cnt++; else continue;

		   				    // Now what do we want to do with the pair?

		   					normy2 = sqrt(normy2);  // Now just radius
		   					normz2 = sqrt(normz2);  // Now just radius
		   				    // Find the radial bin
		   				    int bin1 = floor(norm2/rmax*NBIN);
		   					int bin2 = floor(normy2/rmax*NBIN);
		   					int bin3 = floor(normz2/(2*rmax)*MBIN);
		   		    ...
		   		    // We include the weight for each pair
		   					pairs[thread].add(bin1, bin2, bin3, grid->p[k].w*grid->p[l].w*primary_w);
...
#else
//		    mult[bin1].add(dx.x, dx.y, dx.z, grid->p[k].w);
#endif
...
Particle *read_particles(Float rescale, int *np, const char *filename, const int rstart) {
...
//	if(j%10000==0)	
//	   fprintf(stderr,"%d>=%d %g %g %g %g\n",stat,rstart,tmp[0],tmp[1],tmp[2],tmp[3]);
        if(!(stat==4||stat==5||stat==6))
	...
	if(j%10000==0)
	   fprintf(stderr,"%d %d %d %g %g\n",j,rstart,stat,p[j].w,tmp[stat-1]);

//	   if(j%10000==0)
//		fprintf(stdout,"%d w=%g\n",j,p[j].w);
	}
	j++;
    }
    fclose(fp);
...
int main(int argc, char *argv[]) {
    ...
    printf("Box Size = %6.6g\n", boxsize);
    printf("Grid = %d\n", nside);
    printf("Maximum Radius = %6.6g\n", rmax);
	...
	orig_p = read_particles(rescale, &np, fname,rstart);
	assert(np>0);
    }
