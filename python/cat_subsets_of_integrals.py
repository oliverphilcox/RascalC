...
def cat_subsets_of_integrals(n: int, mstr: str, input_roots: list[str], ns_samples: list[int], output_root: str, collapse_factor: int = 1, print_function = print) -> None:
    ...
    if collapse_factor != 1 and os.path.samefile(input_roots[0], output_root): raise ValueError("Only can collapse samples into a different directory")
    if any(os.path.samefile(input_root, output_root) for input_root in input_roots[1:]): raise ValueError("Only first input directory can be the same as the output")

    n_samples_tot = sum(ns_samples)
    if n_samples_tot % collapse_factor != 0: raise ValueError("Collapse factor must divide the total number of samples")
    ...
    for ii in range(len(I1)): # loop over all field combinations
        ...
        if sum(repeats) < n_samples_tot:
            print_function("ERROR: some %s full samples missing: expected %d, found %d" % (index4, n_samples_tot, sum(repeats)))
        ...
        print_function("Done with %s full" % index4)
        ...
        if sum(repeats) < n_samples_tot:
            print_function("ERROR: some %s jack samples missing: expected %d, found %d" % (index4, n_samples_tot, sum(repeats)))
        ...
        print_function("Done with %s jack" % index4)
        ...
        if sum(repeats) < n_samples_tot:
            print_function("ERROR: some %s disconnected jack samples missing: expected %d, found %d" % (index2, n_samples_tot, sum(repeats)))
            continue # skip the rest of the loop
        ...
        print_function("Done with %s disconnected jack" % index2)
...
if __name__ == "__main__": # if invoked as a script
    ...
    cat_subsets_of_integrals(n, mstr, input_roots, ns_samples, output_root, collapse_factor)