...
def matrix_readin(suffix='full'):
    ...
    for ii in range(len(I1)):
        ...
        c3js[j2,j1,j3]=c3j
        if((j1!=j2)and(j3!=j4)):
            c4s[j1,j2,j3,j4]+=c4*0.5 # to correct for xi_ik xi_jl = xi_il xi_jk assumption
            c4js[j1,j2,j3,j4]+=c4j*0.5
        else:
            c4s[j1,j2,j3,j4]=c4
        ...
        # Add symmetries:
        ## NB: This automatically corrects for the 4-point xi-symmetry assumption
        if j1!=j3:
            c3s[j2,j3,j1]=c3
            c3js[j2,j3,j1]=c3j
        if j1!=j2:
