...
def load_matrices(index):
    ...
    c2 = np.loadtxt(cov_root+'c2_leg_n%d_l%d_11_%s.txt'%(n,max_l,index))
    c3 = np.loadtxt(cov_root+'c3_leg_n%d_l%d_1,11_%s.txt'%(n,max_l,index))
    c4 = np.loadtxt(cov_root+'c4_leg_n%d_l%d_11,11_%s.txt'%(n,max_l,index))
...
output_name = outdir+'Rescaled_Covariance_Matrices_n%d_l%d.npz'%(n,max_l)
np.savez(output_name,covariance=full_cov,
         shot_noise_rescaling=alpha,precision=full_prec,
         N_eff=N_eff_D,D_matrix=full_D_est,
         individual_covariances=partial_cov)
