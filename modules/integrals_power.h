...
public:
    ...
    Integrals(Parameters *par, CorrelationFunction *_cf12, CorrelationFunction *_cf13, CorrelationFunction *_cf24, int _I1, int _I2, int _I3, int _I4, SurveyCorrection *_sc12, SurveyCorrection *_sc23, SurveyCorrection *_sc34, KernelInterp *_kernel_interp){
        ...
        max_legendre = fmax((sc12->l_bins-1)*2,par->max_l);
    ...
    inline void second(const Particle* pi_list, const int* prim_ids, int pln, const Particle pj, const int pj_id, Float* &wij, const double prob, Float* &kernel_ij){
        ...
        Float new_kr, old_kr, new_kernel, diff_kr, new_kr3, old_kr3, legendre[max_legendre/2+1];
        Particle pi;
        int tmp_bin;
        ...
        for(int i=0;i<pln;i++){ // Iterate over particle in pi_list
            ...
            for(int ii=0;ii<nbin;ii++){
                ...
                for(int jj=0;jj<mbin;jj++){
                    ...
                    this_kernel[ii*mbin+jj] = legendre[jj]*(new_kernel - old_kernel[jj])/diff_kr;
    ...
    inline void third(const Particle* pi_list, const int* prim_ids, const int pln, const Particle pj, const Particle pk, const int pj_id, const int pk_id, const Float* wij, Float* &xi_ik, Float* wijk, const double prob, const Float* kernel_ij){
        ...
        Float tmp_weight, xi_ik_tmp, new_kr, old_kr, new_kernel, diff_kr, new_kr3, old_kr3, legendre_jk[max_legendre/2+1];
        int tmp_bin;
        
        printf("Deal with j=k and j-k>R0 cases\n");
        ...
        if(rjk_mag<R0){
            ...
            // but don't discard this yet, since xi_ik is still needed for later
            
            // load legendre polynomials
            legendre_polynomials(rjk_mu,max_legendre,legendre_jk);
        
            // Compute 1/Phi_jk function
            for(int l_i=0;l_i<sc23->l_bins;l_i++) tmp_phi_inv+=legendre[l_i]*sc23->inv_correction_function(l_i*2,rjk_mu);
            
            // Now compute multipole row elements
            for(int ii=0;ii<nbin;ii++){
                if(ii==0){
                    old_kr = rjk_mag*r_high[ii];
                    old_kr3 = pow(old_kr,3);
                }
        ...
        for(int i=0;i<pln;i++){ // Iterate over particle in pi_list
            ...
            tmp_weight = wij[i]*pk.w; // product of weights, w_iw_jw_k *W(r_ij,R0) * Phi_ij
            
            // save arrays for later
            xi_ik[i]=xi_ik_tmp;
            wijk[i]=tmp_weight;
        
            printf("TODO: check j-k too large radius constraints");
            printf("TODO: get inverse Phi for various fields working");
            if(rjk_mag>R0) continue;       
            
            used_triples++; // count a triple
            
            // Now compute the integral;
            c3v = tmp_weight*pj.w*pair_weight(rjk_mag)/prob*xi_ik_tmp*4./tmp_phi_inv; // include symmetry factor
    ...
    inline void fourth(const Particle* pi_list, const int* prim_ids, const int pln, const Particle pj, const Particle pk, const Particle pl, const int pj_id, const int pk_id, const int pl_id, const Float* wijk, const Float* xi_ik, const double prob, const Float* kernel_ij){
        ...
        // First define variables
        Particle pi;
        Float rjl_mag, rjl_mu, rkl_mag, rkl_mu, c4v, xi_jl, tmp_weight, tmp_phi_inv=0,this_kernel[nbin*mbin];
        Float old_kernel[mbin], new_kr, old_kr, new_kernel, diff_kr, new_kr3, old_kr3, legendre_kl[max_legendre/2+1];
        int tmp_bin;
        
        cleanup_l(pl.pos,pk.pos,rkl_mag,rkl_mu); 
        if(rkl_mag>R0) continue; // if k-l separation too large
        
        // Load legendre polynomials
        legendre_polynomials(rkl_mu, max_legendre, legendre_kl);
        
        // Compute 1/Phi_kl function
        for(int l_i=0;l_i<sc34->l_bins;l_i++) tmp_phi_inv+=legendre[l_i]*sc34->inv_correction_function(l_i*2,rkl_mu);
        
        // Now compute multipole matrix elements
        for(int ii=0;ii<nbin;ii++){
            if(ii==0){
                old_kr = rkl_mag*r_high[ii];
                old_kr3 = pow(old_kr,3);
            }
        ...
        for(int i=0;i<pln;i++){ // Iterate over particle in pi_list
            ...
            // Now compute the integral;
            c4v = tmp_weight*pl.w*pair_weight(rkl_mag)/(tmp_phi_inv*prob)*2.*xi_ik[i]*xi_jl; // with xi_ik*xi_jl = xi_il*xi_jk symmetry factor
...
public:
    ...
    void sum_counts(Integrals *ints){
