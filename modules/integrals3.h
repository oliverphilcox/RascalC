...
#ifndef INTEGRALS_3_H
#define INTEGRALS_3_H
...
public:
    ...
    inline void second(const Particle* pi_list, const int* prim_ids, int pln, const Particle pj, const int pj_id, Float* &xi_ij, int* &bin, Float* &wij, const double prob){
        ...
        for(int i=0;i<pln;i++){ // Iterate over particle in pi_list
                ...
                Float JK_weight2;
                JK_weight2=weight_tensor2(int(pi.JK),int(pj.JK),int(pi.JK),int(pj.JK),tmp_bin,tmp_bin);
            
                // Now compute the integral:
                c2vj = tmp_weight*tmp_weight*(1.+tmp_xi)/prob*JK_weight2;
    ...
    inline void third(const Particle* pi_list, const int* prim_ids, const int pln, const Particle pj, const Particle pk, const int pj_id, const int pk_id, const int* bin_ij, const Float* wij, Float* &xi_jk, Float* &xi_ik, Float* wijk, const double prob){
        ...
        for(int i=0;i<pln;i++){ // Iterate ovr particle in pi_list
            ...
            // Compute jackknife weight tensor:
            Float JK_weight2;
            JK_weight2=weight_tensor2(int(pi.JK),int(pj.JK),int(pk.JK),int(pi.JK),bin_ij[i],tmp_bin);
            
            // Now compute the integral;
            c3v = tmp_weight*pi.w/prob*xi_jk_tmp;
            c3vj = tmp_weight*pi.w/prob*xi_jk_tmp*JK_weight2;
    ...
    inline void fourth(const Particle* pi_list, const int* prim_ids, const int pln, const Particle pj, const Particle pk, const Particle pl, const int pj_id, const int pk_id, const int pl_id, const int* bin_ij, const Float* wijk, const Float* xi_ik, const Float* xi_jk, const Float* xi_ij, const double prob){
        ...
        for(int i=0;i<pln;i++){ // Iterate over particle in pi_list
            ...
            // Compute jackknife weight tensor:
            Float JK_weight, JK_weight2;
            JK_weight=weight_tensor(int(pi.JK),int(pj.JK),int(pk.JK),int(pl.JK),bin_ij[i],tmp_bin);
            JK_weight2=weight_tensor2(int(pi.JK),int(pj.JK),int(pk.JK),int(pl.JK),bin_ij[i],tmp_bin);
            
            // Now compute the integral;
            c4v = tmp_weight/prob*(xi_il*xi_jk[i]+xi_jl*xi_ik[i]);
            c4vj = tmp_weight/prob*(xi_il*xi_jk[i]+xi_jl*xi_ik[i])*JK_weight2;
...
private:   
    ...
    private:   
    inline Float weight_tensor2(const int Ji, const int Jj, const int Jk, const int Jl, const int bin_a, const int bin_b){
        // Compute the jackknife weight tensor for jackknife regions J_i, J_j, J_k, J_l and w_aA weight matrix w_matrix. NB: J_x are collapsed jackknife indices here - only using the non-empty regions.
        // JK_weights class holds list of filled JKs, number of filled JKs and the weight matrix.
        // bin_a, bin_b specify the binning.
        ...
        Float total_weight = (Float)first_weight/4.;
...
public:
    ...
    /* Print integral outputs to file. 
        ...
        */
        // Create output files
        char c2name[1000];
        snprintf(c2name, sizeof c2name, "ModCovMatricesAll/c2_n%d_m%d_%s.txt", nbin, mbin,suffix);
        char c3name[1000];
        snprintf(c3name, sizeof c3name, "ModCovMatricesAll/c3_n%d_m%d_%s.txt", nbin, mbin, suffix);
        char c4name[1000];
        snprintf(c4name, sizeof c4name, "ModCovMatricesAll/c4_n%d_m%d_%s.txt", nbin, mbin, suffix);
        char c4errname[1000];
        snprintf(c4errname, sizeof c4errname, "ModCovMatricesAll/c4err_n%d_m%d_%s.txt", nbin, mbin, suffix);
        char RRname[1000];
        snprintf(RRname, sizeof RRname, "ModCovMatricesAll/RR_n%d_m%d_%s.txt", nbin, mbin, suffix);
    ...
    /* Print jackknife integral outputs to file. 
        ...
        */
        // Create output files
        char c2name[1000];
        snprintf(c2name, sizeof c2name, "ModCovMatricesJack/c2j_n%d_m%d_%s.txt", nbin, mbin,suffix);
        char c3name[1000];
        snprintf(c3name, sizeof c3name, "ModCovMatricesJack/c3j_n%d_m%d_%s.txt", nbin, mbin, suffix);
        char c4name[1000];
        snprintf(c4name, sizeof c4name, "ModCovMatricesJack/c4j_n%d_m%d_%s.txt", nbin, mbin, suffix);
        char cxname[1000];
        snprintf(cxname, sizeof cxname, "ModCovMatricesJack/cxj_n%d_m%d_%s.txt", nbin, mbin, suffix);
        char c4errname[1000];
        snprintf(c4errname, sizeof c4errname, "ModCovMatricesJack/c4errj_n%d_m%d_%s.txt", nbin, mbin, suffix);
        char cxerrname[1000];
        snprintf(cxerrname, sizeof cxerrname, "ModCovMatricesJack/cxerrj_n%d_m%d_%s.txt", nbin, mbin, suffix);
        FILE * C2File = fopen(c2name,"w"); // for c2 part of integral
        FILE * C3File = fopen(c3name,"w"); // for c3 part of integral
        FILE * C4File = fopen(c4name,"w"); // for c4 part of integral
