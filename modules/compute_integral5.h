    ...
    #ifndef COMPUTE_INTEGRAL5_H
    #define COMPUTE_INTEGRAL5_H

    #include "integrals4.h"

    class compute_integral5{
    ...
    public:    
        compute_integral5(Grid *grid, Parameters *par, JK_weights *JK){
    ...
    //-----------INITIALIZE OPENMP + CLASSES----------
            ...
            Integrals4 sumint(par,cf,JK); // total integral
            ...
            uint64 fln_sum=0; // total number of l particles used
            Float sln_norm=0,sln_sum=0.;
    ...
    #ifdef OPENMP       
    #pragma omp parallel firstprivate(steps) shared(sumint) reduction(+:convergence_counter,cell_attempt2,cell_attempt3,cell_attempt4,sln_sum,sln_norm,fln_sum,used_cell2,used_cell3,used_cell4)
    #endif
            { // start parallel loop
    ...
    //-----------DEFINE VARIABLES
            ...
            Particle *fou_list,*sec_list; // list of particles in l cell
            int *fou_ids, fou_id_1D, *sec_ids, sec_id_1D; // list of particle ids in l cell
            
            Integrals4 locint(par,cf,JK); // Accumulates the integral contribution of each thread
            ...
            ec+=posix_memalign((void **) &fou_ids, PAGE, sizeof(int)*mnp);
            ec+=posix_memalign((void **) &sec_list, PAGE, sizeof(Particle)*mnp);
            ec+=posix_memalign((void **) &sec_ids, PAGE, sizeof(int)*mnp);
    ...
    #pragma omp for schedule(dynamic)
            ...
            for (int n_loops = 0; n_loops<par->max_loops; n_loops++){
                ...
                percent_counter=0.;
                printf("\nNEED TO INCLUDE QUAD COUNTS HERE PROPERLY\n");
                ...
                for (int n1=0; n1<grid->nf;n1++){
                    ...
                    for (int n2=0; n2<par->N2; n2++){
                        ...
                        // Draw fourth cell from i cell weighted by 1/r^2
                        delta2 = rd->random_cubedraw(locrng, &p2);
                        sec_id = prim_id+delta2;
                                
                        sec_id_1D = grid-> test_cell(sec_id); 
                        if(sec_id_1D<0) continue; // error if cell not in grid
                                
                        sln = particle_list(sec_id_1D,sec_list,sec_ids,grid);
                        sln_sum+=sln*p2;
                        sln_norm+=p2;
                                
                        used_cell2+=1; // new second cell used
                                
                        p2*=1./(grid->np*(double)sln);
                                
                        locint.second(prim_list, prim_ids, pln, sec_list, sec_ids, sln, bin_ij, w_ij, p2);
                        
                        pid_j=sec_ids[0];
                        particle_j=sec_list[0];
                        ...
                        for (int n3=0; n3<par->N3; n3++){
                            ...
                            for (int n4=0; n4<par->N4; n4++){
                                ...
                                fou_id_1D = grid-> test_cell(fou_id); 
                                if(fou_id_1D<0) continue; // error if cell not in grid
                                
                                fln = particle_list(fou_id_1D,fou_list,fou_ids,grid);
                                fln_sum+=fln;
                                ...
                                locint.fourth(prim_list, prim_ids, pln, fou_list, fou_ids, fln, particle_j, particle_k, pid_j, pid_k, bin_ij, w_ijk, xi_ik, xi_jk, p4);
    ...
    //-----------REPORT + SAVE OUTPUT---------------
        ...
        Float grid_density = double(sln_sum)/double(grid->nf*par->N2*par->max_loops);//(double)par->np/grid->nf;//pow(par.nside,3);
        printf("\nSampling density in j cell: %.2f\n",grid_density);
        printf("\nsln sum: %.2e\n",sln_sum/sln_norm);
        printf("\nFull density: %.2e\n",sln_sum/sln_norm/double(grid->nf));
        printf("\nAll density: %.2e\n",sln_sum/sln_norm/double(grid->ncells));
        
        //tot_pairs*=grid_density;
