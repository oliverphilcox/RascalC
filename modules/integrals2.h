...
//  
    ...
    inline void third(const Particle* pi_list, const int* prim_ids, const int pln, const Particle pj, const Particle pk, const int pj_id, const int pk_id, const int* bin_ij, const Float* wij, Float* &xi_ik, Float* wijk, const double prob){
        ...
        for(int i=0;i<pln;i++){ // Iterate ovr particle in pi_list
            ...
            // Compute jackknife weight tensor:
            JK_weight=weight_tensor(int(pi.JK),int(pj.JK),int(pj.JK),int(pk.JK),bin_ij[i],tmp_bin);
...
public:
    ...
    void normalize(int np, int ngal, Float n_pairs, Float n_triples, Float n_quads, bool use_RR){
        ...
        for(int i = 0; i<nbin*mbin;i++){
            ...
            errc2[i]/=(n_pairs*corrf4);
            errc2j[i]/=(n_pairs*corrf4);
            for(int j=0;j<nbin*mbin;j++){
        ...
        if(use_RR==1){
            ...
            for(int i=0; i<nbin*mbin;i++){
                ...
                errc2[i]/=pow(Ra_i,4.);
                errc2j[i]/=pow(Ra_i,4.)*pow(1.-JK->product_weights[i*nbin*mbin+i],2.);
                for(int j=0;j<nbin*mbin;j++){
    ...
    /* Print integral outputs to file. 
        ...
        */
        ...
        char c2errname[1000];
        snprintf(c2errname, sizeof c2errname, "%sCovMatricesAll/c2err_n%d_m%d_%s.txt", out_file, nbin, mbin, suffix);
        char c4errname[1000];
...
//             fclose(RR2File);
    ...
    /* Print jackknife integral outputs to file. 
        ...
        */
        ...
        char c2errname[1000];
        snprintf(c2errname, sizeof c2errname, "%sCovMatricesJack/c2errj_n%d_m%d_%s.txt", out_file, nbin, mbin, suffix);
        FILE * C2File = fopen(c2name,"w"); // for c2 part of integral
        FILE * C3File = fopen(c3name,"w"); // for c3 part of integral
        FILE * C4File = fopen(c4name,"w"); // for c4 part of integral
