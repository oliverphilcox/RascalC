...
#pragma omp for schedule(dynamic,32)
    ...
    static int xi_integrand(unsigned ndim, const double *x, void *fdata, unsigned fdim, double *fval){
        ...
        if(n<=0){
            ...
            fval[0]= pow(x[0],2)/ (pow(R,3))*exp(-pow(x[0]/(2*R),2))*(abs(corr->xi(x[0]))+pow(r_min,2.)/pow(x[0]+r_min,4.)) ;
        } else{ 
            // Use full expression for non-zero n
            fval[0] = x[0]/ (R*n) * (exp(-factor_2)-exp(-factor_1))*(abs(corr->xi(x[0]))+pow(r_min,2.)/pow(x[0]+r_min,4.)) ;
        }
