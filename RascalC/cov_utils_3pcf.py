...
def convert_cov_3pcf_legendre(cov: npt.NDArray[np.float64], max_l: int, exclude_odd_l: bool = False, apply_scaling: bool = True) -> npt.NDArray[np.float64]:
    ...
    if apply_scaling: # prepare to apply the ell-dependent factor between RascalC and ENCORE basis functions
        ...
        ell_factor = ((-1)**ells * np.sqrt(2 * ells + 1) / (4 * np.pi)) # the ell-dependent factor between the ENCORE 3-point basis functions and Legendre polynomials given by Equation (17) in https://arxiv.org/pdf/2105.08722. RascalC uses Legendre polynomial basis according to Equation (4.11) in https://arxiv.org/pdf/1910.04764
