...
def jack_realization_rascalc(jack_estimator: lsstypes.Count2JackknifeCorrelation, ii: int) -> lsstypes.Count2Correlation:
    ...
    for count_name in jack_estimator.count_names:
        ...
        new_counts._data['counts'] = jack_counts.get(realizations=ii, cross='ii').values('counts') + 0.5 * (jack_counts.get(realizations=ii, cross='ij').values('counts') + jack_counts.get(realizations=ii, cross='ji').values('counts')) # j1 x all2 + all1 x j2 = 2 x j1 x j2 + j1 x (all2 - j2) + j2 x (all1 - j1); by conventions from jackknife_weights{,_cross}.py needs to be divided by 2
        new_counts._data['norm'] = jack_counts.values('norm') # we want the norm to be the same as for the total counts for further RascalC usage, otherwise the normalized counts of the jackknife realizations would not sum to the total normalized counts
        new_counts._data['normalized_counts'] = new_counts._data.pop('counts') / new_counts._data['norm'] # normalized_counts are the primary value in lsstypes, whereas counts are not
