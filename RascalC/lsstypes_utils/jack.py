...
def jack_realization_rascalc(jack_estimator: lsstypes.Count2JackknifeCorrelation, ii: int) -> lsstypes.Count2Correlation:
    ...
    for count_name in jack_estimator.count_names:
        ...
        kw[count_name] = lsstypes.Count2(counts=counts, norm=jack_counts.values('norm')) # we want the norm to be the same as for the total counts for further RascalC usage, otherwise the normalized counts of the jackknife realizations would not sum to the total normalized counts
