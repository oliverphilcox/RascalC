...
def sample_cov_from_lsstypes(xi_estimators: list[list[lsstypes.Count2Correlation]]) -> npt.NDArray[np.float64]:
    r"""
    ...
    xi = np.array([[xi_estimator.value() for xi_estimator in xi_estimators_c] for xi_estimators_c in xi_estimators])
...
def sample_cov_from_lsstypes_to_file(xi_estimators: list[list[lsstypes.Count2Correlation]], outfile_name: str) -> None:
    r"""
    ...
    """
    np.savetxt(outfile_name, sample_cov_from_lsstypes(xi_estimators))


def sample_cov_from_lsstypes_files(infile_names: list[list[str]], outfile_name: str) -> None:
    r"""
    ...
    sample_cov_from_lsstypes_to_file(xi_estimators, outfile_name)
