...
parser = argparse.ArgumentParser(description="This script reads two sets of RascalC results (in radial/separation and angular/mu bins) and two cosmodesi/pycorr .npy files to combine two covs following NS/GCcomb procedure for 2 tracers. Covariance of N and S 2PCF is neglected.")
...
combine_covs_multi(args.rascalc_files[0], args.rascalc_files[1], [args.pycorr_files[i] for i in range(0, 6, 2)], [args.pycorr_files[i] for i in range(1, 6, 2)], args.output_cov_file, args.n_mu_bins or None, args.r_step, args.skip_r_bins, args.output_cov_file1, args.output_cov_file2)